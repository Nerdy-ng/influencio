...
async function sendTikTokServerEvent(event, email, properties = {}) {
  try {
    await fetch('/api/tiktok-event', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ event, email, url: window.location.href, properties }),
    })
  } catch (err) {
...
export default function App() {
  ...
  // TikTok funnel — ViewContent on mount (browser + server)
  useEffect(() => {
    if (window.ttq) window.ttq.track('ViewContent', { content_name: 'Brandior Waitlist', content_type: 'product' })
    sendTikTokServerEvent('ViewContent', null, { content_name: 'Brandior Waitlist', content_type: 'product' })
  }, [])
  ...
  async function handleSubmit(e) {
    ...
    // Server-side Lead event with hashed email for TikTok matching
    sendTikTokServerEvent('Lead', email.trim().toLowerCase(), { content_name: role, content_type: 'product' })
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ backgroundColor: '#160035' }}>
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4 py-10">
        ...
        {/* Badge */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full -mt-2 mb-9 text-xs font-semibold tracking-wide uppercase"
        ...
        <h1 className="text-4xl sm:text-6xl font-black text-center leading-[1.1] mb-4 max-w-2xl" style={{ fontFamily: "'Satoshi', sans-serif" }}>
          <span className="text-white">Where Creators & Brands</span>
          <br />
          <span style={{ color: '#FA8112' }}>Close Deals.</span>
        </h1>
        ...
        <div className="w-full max-w-md">
          {state === 'success' ? (
            <div className="rounded-3xl p-8 text-center" style={{ backgroundColor: 'rgba(236,72,153,0.15)', border: '1px solid rgba(236,72,153,0.4)', backdropFilter: 'blur(20px)', animation: 'slide-up 0.5s ease' }}>
              ...
              <p className="text-xs font-semibold mb-6" style={{ color: '#FA8112' }}>
              ...
              <div className="pt-6 border-t" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                {role === 'creator' ? (
                  <>
                    <p className="text-sm font-semibold text-white mb-1">Join our WhatsApp community</p>
                    <p className="text-xs mb-4" style={{ color: 'rgba(255,255,255,0.5)' }}>Get insider updates and be first to know when we launch</p>
                    <a
                      href="https://chat.whatsapp.com/IizvwKcuqJNHjSpGp8LXHG"
                      target="_blank"
                  ...
                  <>
                    <p className="text-xs mb-3 text-white">Share with your network</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div>
                  <input
                    ...
                    onFocus={e => { e.target.style.borderColor = 'rgba(124,58,237,0.6)'; if (window.ttq) window.ttq.track('AddToCart', { content_name: 'Brandior Waitlist', content_type: 'product' }) }}
        ...
        <div className="mt-10 max-w-xl w-full rounded-2xl p-7 text-center">
          <p className="text-sm mb-5 text-white">
            Don't Shine in the Dark. Get <span style={{ color: '#FA8112' }}>SEEN.</span>
          </p>
        ...
        <div className="mt-14 max-w-2xl w-full">
          <p className="text-center text-xs font-semibold uppercase tracking-widest mb-6 text-white">No agency friction. No hidden charges. 100% free registration.</p>
